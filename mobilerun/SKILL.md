...
- **Profiles** — device specification presets (hardware, locale). Cloud Phones only.
- **Proxy Configs** — saved SOCKS5 proxy configurations. A proxy must be configured before provisioning Cloud or Physical Phones.
- **Carriers** — mobile carrier configurations for eSIM.
- **eSIM** — cellular connectivity for Physical Phones. Configure through the device's eSIM section in the dashboard.
